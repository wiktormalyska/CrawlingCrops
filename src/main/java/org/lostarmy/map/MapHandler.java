...
public class MapHandler {
    ...
    public void generateCaveMap() {
        ...
        while (areFreePoints()) {
            ...
            int connectingPoints = (int) (Math.random() * 3) + 1;
            for (int i = 0; i < connectingPoints; i++) {
                Cell center2 = findClosest(center.getX(), center.getY());
    ...
    public void drawLine(int x1, int y1, int x2, int y2) {
        ...
        while (true) {
            ...
            map[x1][y1] = new Marker(x1, y1, ConsoleColors.BLUE);
            map[x1 + 1][y1] = new Marker(x1, y1, ConsoleColors.BLUE);
            map[x1 - 1][y1] = new Marker(x1, y1, ConsoleColors.BLUE);
            map[x1][y1 + 1] = new Marker(x1, y1, ConsoleColors.BLUE);
            map[x1][y1 - 1] = new Marker(x1, y1, ConsoleColors.BLUE);
            if (x1 == x2 && y1 == y2) {
                break;
            }
